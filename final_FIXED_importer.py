#!/usr/bin/env python3
"""
DEFINITIVE WORKING SOLUTION (v2) - Corrects the typo in the activity party property name.
This script imports all emails with correct sender, formatting, and status.
"""
...
def get_auth_headers():
    ...
    if "access_token" not in result:
        print(f"❌ Auth failed. Full MSAL response: {result}")
        return None
...
if __name__ == "__main__":
    print("🎬 Definitive Import Script Initialized (v4 - Fresh Token Fix)")
    print("==========================================================")
    
    # Perform initial auth to get IDs
    print("\n[Phase 1] Verifying contact and user IDs...")
    initial_headers = get_auth_headers()
    if not initial_headers: exit(1)
    
    rc_contact_id = get_contact_id(initial_headers, 'service@ringcentral.com')
    user_id = get_systemuser_id(initial_headers, 'gvranjesevic@dynamique.com')
    if not rc_contact_id or not user_id: exit(1)

    print("\n🧹 NOTE: Running this script multiple times will create duplicate emails.")
    print("   Please manually delete previous test runs from Dynamics before proceeding.")
    ...
    if input(f"\n🤔 Ready to import {len(april_emails)} emails with the FINAL, correct method? (yes/no): ").lower() == 'yes':
        # FIX: Get a fresh token right before the long-running import process.
        print("\n[Phase 2] Acquiring fresh token for import...")
        import_headers = get_auth_headers()
        if not import_headers:
            print("❌ Could not get a fresh token for the import. Aborting.")
            exit(1)
            
        total_errors = import_emails(import_headers, rc_contact_id, user_id, april_emails)
        if total_errors == 0:
            print("\n🎉🎉🎉 All problems solved. The timeline has been fully restored. 🎉🎉🎉")
        else:
            print(f"\n⚠️ The import completed with {total_errors} errors.")
    else:
